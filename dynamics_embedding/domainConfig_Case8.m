...
function [domain] = domainConfig_Case8(type,opt)
...
controller.opts = optimset('Display','off','Algorithm', 'interior-point-convex', 'TolX',1e-6,'TolFun',1e-6);
% controller.opts = optimset('Display','off','Algorithm', 'active-set',...
...
ctrl_method = opt.ctrl_method; %'QP-CLF' 'QP-MinNorm' 'IO'

switch type
    case 1 %%%%%%%%%%%%%%%%%%%%%%% For Case8: One step
        domain.time = opt.dt1;
%         domain.kp_energy = 100;
        domain.controllerType = ctrl_method;
        ...
        penalty = 5*ones(1,nClf); %% Check later
